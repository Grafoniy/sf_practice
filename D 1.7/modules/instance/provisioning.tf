resource "null_resource" "docker-swarm-manager" {
  ...
  provisioner "file" {
    source      = "/shop/yandex/docker-compose/docker-compose-v3.yml"
...
resource "null_resource" "docker-swarm-manager-start" {
  ...
  provisioner "remote-exec" {
    inline = [
        "docker stack deploy --with-registry-auth -c ~/docker-compose.yml sockshop-swarm"
    ]
  }
